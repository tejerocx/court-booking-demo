  ...
  const note = p.note?.trim()
    ? `<div style="background:#fffbea;border:1.5px solid #f0d060;border-radius:10px;padding:14px 18px;margin-bottom:20px;">
        <div style="font-size:.82rem;color:#7a6020;line-height:1.6;">
          <strong>📝 Message from Your Court Name:</strong><br/>${p.note}
        </div>
       </div>`
    : "";
...
<title>Booking Rescheduled — Your Court Name</title>
</head>
  ...
  <tr><td align="center">
    <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:14px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08);max-width:560px;width:100%;">
      ...
      <tr><td style="background:linear-gradient(135deg,#2d7a2d,#1a4a1a);padding:32px 36px;text-align:center;">
        ...
        <div style="font-family:'Bebas Neue',Georgia,serif;font-size:1.6rem;letter-spacing:3px;color:#fff;line-height:1.1;">YOUR COURT NAME</div>
        <div style="font-size:.75rem;color:rgba(255,255,255,.7);letter-spacing:2px;text-transform:uppercase;margin-top:4px;">Your Location</div>
      </td></tr>
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const fromAddress = Deno.env.get("EMAIL_FROM") || "Your Court Name <onboarding@resend.dev>";
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: fromAddress,
        to: [body.email],
        subject: `📅 Booking Rescheduled — ${body.bookingRef} | Your Court Name`,
        html: buildHtml(body),
      }),
    });
